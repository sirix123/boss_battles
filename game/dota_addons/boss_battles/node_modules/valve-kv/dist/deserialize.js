...
exports.DeserializationError = exports.deserializeFile = exports.deserialize = void 0;
const fs = require("fs");
const path = require("path");
const index_1 = require("./index");
function deserialize(kvstring, encoding = "utf8") {
    const buffer = typeof kvstring === "string" ? Buffer.from(kvstring) : kvstring;
    const parser = new Parser(buffer, encoding);
    return parser.parse();
}
exports.deserialize = deserialize;
function deserializeFile(filename, encoding = "utf8") {
...
class Parser {
    constructor(buffer, encoding, workingDir) {
        this.buffer = buffer;
    ...
    parse() {
        ...
        for (const baseIncluded of baseIncludes) {
            ...
            if (includedValues.length > 0 && index_1.isKvObject(includedValues[0])) {
    ...
    parseBases() {
        ...
        if (this.next === "#" && this.workingDir === undefined) {
            throw new DeserializationError("#base includes are only supported when using deserializeFile", this.index);
        }
        while (this.next === "#") {
    ...
    lookback(count) {
        return this.buffer.toString(this.encoding, this.index - count * this.characterSize, this.index - (count - 1) * this.characterSize);
    }
    atEOF() {
    ...
    step() {
        ...
        if (this.atEOF()) {
            ...
            this.next = this.buffer.toString(this.encoding, this.index, this.index + this.characterSize);
        }
    }
    skipWhitespace() {
        // Ignore whitespace
...
function assignOrMerge(obj, key, value) {
    ...
    if (obj[key] !== undefined) {
        ...
        if (Array.isArray(currentValue)) {
            ...
            obj[key] = index_1.createDuplicateKeyArray([currentValue, value]);
        }
    }
    else {
        obj[key] = value;
