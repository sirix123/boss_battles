...
function clock_hookshot_modifier:OnDestroy( kv )
    if IsServer() then
        ...
        self.target:AddNewModifier(self:GetCaster(), self, "stunned_modifier", { distance = self:GetAbility():GetSpecialValueFor( "knock_stun_duration" ) } )
        ...
        -- leap
        local arc = self.target:AddNewModifier(
