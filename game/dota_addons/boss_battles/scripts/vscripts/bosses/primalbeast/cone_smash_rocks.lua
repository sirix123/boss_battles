...
function cone_smash_rocks:OnAbilityPhaseStart()
    if IsServer() then
...
function cone_smash_rocks:OnSpellStart()
    if IsServer() then
        ...
        -- root the boss, remove root at the end of the timers

        -- DebugDrawCircle(origin, Vector(0,0,255), 128, 50, true, 60)
        ...
        Timers:CreateTimer(function()
            ...
            -- play animation
            -- point that comes in from the AI file... use that to point the boss for each wave
            ...
            -- self:GetCaster():SetForwardVector(face_point)
            -- self:GetCaster():FaceTowards(face_point)
