...
function TimberThink()
	...
	if thisEntity:GetHealthPercent() < 85 and thisEntity.chain_map_edge:IsCooldownReady() == true then
		...
		thisEntity.state = 2
	elseif thisEntity.state == 2 and ( FindUnitsCloseAndBubbleGone() == true or self.end_phase_2 == true ) then
	...
	if thisEntity.state == 2 then
		...
		if thisEntity.createParticleOnce == true then
			...
			-- start a timer, if this timer ends then we end phase 2... 
			-- this phase only lats 45seconds now (MAX)
			print("end phase 2 - 2")
			self.end_phase_2 = false
			Timers:CreateTimer(15, function()
				print("end phase 2 - 1")
				if IsValidEntity(thisEntity) == false then return false end
				self.end_phase_2 = true
				return false
			end)
