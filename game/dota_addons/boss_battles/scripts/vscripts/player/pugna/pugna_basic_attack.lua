pugna_basic_attack = class({})

function pugna_basic_attack:GetCastPoint()
...
function pugna_basic_attack:OnAbilityPhaseStart()
    if IsServer() then
...
function pugna_basic_attack:OnAbilityPhaseInterrupted()
    if IsServer() then
...
function pugna_basic_attack:OnSpellStart()
    if IsServer() then
        ...
        local projectile = {
            EffectName = "particles/pugna/pugna_pugna_base_attack.vpcf",
            ...
            OnUnitHit = function(_self, unit)
                ...
                EmitSoundOnLocationWithCaster(unit:GetAbsOrigin(), "hero_Pugna.projectileImpact", self.caster)

            end,
            OnFinish = function(_self, pos)
                -- add projectile explode particle effect here on the pos it finishes at
                local particle_cast = "particles/units/heroes/hero_pugna/pugna_base_attack_explosion.vpcf"
